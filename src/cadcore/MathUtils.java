...
public class MathUtils {
	...
	public static class RootFinder {
		...
		static public double getRoot(Function function, double targetValue, double minLimit, double maxLimit) {
			...
			if (Math.abs(sec_actualValue - targetValue) > ROOTFINER_VALUE_TOLERANCE) {
				// System.out.printf("getRoot(): SecantRootFinder failed, x:%f,
				// value:%f, targetValue:%f, error: %f\n", x, function.f(x),
				// targetValue, function.f(x)-targetValue);
				...
				if (Math.abs(bis_actualValue - targetValue) > ROOTFINER_VALUE_TOLERANCE) {
					// System.out.printf("getRoot(): BisectRootFinder failed,
					// x:%f, value:%f, targetValue:%f, error: %f\n", x,
					// function.f(x), targetValue, function.f(x)-targetValue);

					if (Math.abs(bis_actualValue - targetValue) < Math.abs(sec_actualValue - targetValue)) // Test
																											// which
																											// is
																											// closest
																											// and
					...
					{
						x = bis_x;
						// System.out.printf("getRoot(): Best result with
						// bisect, x:%f, value:%f, targetValue:%f, error: %f\n",
						// x, bis_actualValue, targetValue,
						// bis_actualValue-targetValue);
					} else {
						// System.out.printf("getRoot(): Best result with
						// secant, x:%f, value:%f, targetValue:%f, error: %f\n",
						// x, sec_actualValue, targetValue,
						// sec_actualValue-targetValue);
					}
				} else {
					x = bis_x;
				}
		...
		static public double getRoot(DerivableFunction function, double targetValue, double minLimit, double maxLimit) {
			...
			if (Math.abs(function.f(x) - targetValue) > ROOTFINER_VALUE_TOLERANCE) {
				// System.out.printf("getRoot(): SecantRootFinder failed, error:
		...
		public static class NewtonRaphsonRootFinder {
			...
			static public double getRoot(DerivableFunction function, double target_value, double minLimit,
					double maxLimit) {
				// Guess initial value
		...
		public static class SecantRootFinder {
			...
			static public double getRoot(Function function, double target_value, double minLimit, double maxLimit) {
				...
				// System.out.printf("SecantRootFinder.getRoot(): START
				// current_error:%f, x:%f last_value:%f current_value:%f
				// target_value:%f\n", current_error, x, last_value,
				...
				while (Math.abs(current_error) > ROOTFINER_VALUE_TOLERANCE && n++ < SECANT_MAX_ITERATIONS
					...
					// System.out.printf("SecantRootFinder.getRoot(): d:%f,
					// current_error:%f, x:%f last_value:%f current_value:%f
					// target_value:%f\n", d, current_error, x, last_value,
					// current_value, target_value);

				}
				// System.out.printf("SecantRootFinder.getRoot():
				// target_value:%f, current_value:%f current_error:%f\n",
		...
		public static class BisectRootFinder {
			...
			static public double getRoot(Function function, double target_value, double minLimit, double maxLimit) {
				...
				// return 0.0; //NO ROOT WITHIN LIMITS? FUNCTION NOT SUITABLE
				// FOR BISECT METHOD

				double current_error = 100000000;
				double x = 0;
				while (Math.abs(current_error) > ROOTFINER_VALUE_TOLERANCE && n++ < BISECT_MAX_ITERATIONS
						&& ht - lt > 0.0001) {
					x = (ht + lt) / 2.0;
					...
					if (current_error * l_error < 0.0) // if these two positions
														// have the opposite
														// sign, then the root
														// must be within the
														// this span
					...
					// System.out.printf("BisectRootFinder.getRoot():
					// current_error:%f, x:%f lt:%f ht:%f current_value:%f
					// target_value:%f \n", current_error, x, lt, ht,
					// current_value, target_value);
				}
	...
	static public double[][] invert(double[][] a) {
		...
		double det = a[0][0] * (a[2][2] * a[1][1] - a[2][1] * a[1][2])
				- a[1][0] * (a[2][2] * a[0][1] - a[2][1] * a[0][2]) + a[2][0] * (a[1][2] * a[0][1] - a[1][1] * a[0][2]);

		double[][] b = {
				{ (a[2][2] * a[1][1] - a[2][1] * a[1][2]) / det, -(a[2][2] * a[0][1] - a[2][1] * a[0][2]) / det,
						(a[1][2] * a[0][1] - a[1][1] * a[0][2]) / det },
				{ -(a[2][2] * a[1][0] - a[2][0] * a[1][2]) / det, (a[2][2] * a[0][0] - a[2][0] * a[0][2]) / det,
						-(a[1][2] * a[0][0] - a[1][0] * a[0][2]) / det },
				{ (a[2][1] * a[1][0] - a[2][0] * a[1][1]) / det, -(a[2][1] * a[0][0] - a[2][0] * a[0][1]) / det,
