...
import boardcam.holdingsystems.TableBlockHoldingSystem;
...
import boardcad.gui.jdk.FasterBrd3DModelGenerator;
...
public class MachineConfig extends CategorizedSettings {
	...
	FasterBrd3DModelGenerator mBlank3DModelGenerator = new FasterBrd3DModelGenerator();
	...
	public void initialize() {
		...
		Settings.SettingChangedCallback onBlankHoldingSystemChanged = new Settings.SettingChangedCallback() {
			public void onSettingChanged(Setting setting) {
				...
				case 2:
					...
					break;
				case 3:
					mBlankHoldingSystem = new TableBlockHoldingSystem(MachineConfig.this);
					if (generalSettings.getEnumeration(CUTTER_TYPE) == 2) {
						generalSettings.setEnumeration(CUTTER_TYPE, 0);
					}
					generalSettings.getSetting(CUTTER_TYPE).signal();
					blankSetting.setHidden(true);
					blankSetting.signal();
					break;
				}

				mBlankHoldingSystem.setChangeListener(new ChangeListener() {
					...
					public void onChange() {
						if (mMachineView != null) {
							...
							mMachineView.get3DView().setBrdBottomOffset(new Point3d(mBlankHoldingSystem.getBoardBottomOffsetPos()), mBlankHoldingSystem.getBoardBottomOffsetAngle());
						}
					}
				});
		...
		holdingSystemTypes.put(3, LanguageResource.getString("BLOCKTABLE_STR"));
		...
		generalSettings.addFileName(BLANK, "", LanguageResource.getString("BLANK_STR"), new Settings.SettingChangedCallback() {
			public void onSettingChanged(Setting setting) {
				...
				if (!generalSettings.getSetting(BLANK).isHidden()) {
		 ...
		 * generalSettings.addAction(SAVE_G_CODE, "...", "Save g-code", new
		...
		// homeingTypes.put(3, LanguageResource.getString("HOME_Z_STR")); TODO:
		// Dunno how to do home on z only, have to wait
		...
		// Read cutter and blank holding system settings here so we don't
		// overwrite with default values when getting the full set of settings
		// result in changed of cutter or holding system
