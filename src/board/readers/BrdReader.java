...
public class BrdReader {
	...
	static public int loadEncryptedFile(BezierBoard brd, String aFilename, String key) {
		...
		try {
			...
			PBEParameterSpec pbeParamSpec = new PBEParameterSpec(new byte[] { -57, 115, 33, -116, 126, -56, -18, -103 },
					20);
			...
			while (strLine != null) {
				...
				case 35: // Slices
				{
					...
					while (strLine.startsWith("(p36")) {
						...
						if (b < 0)
							b = strLine.length();
						// String value = strLine.substring(a,
						// strLine.length());
	...
	static void readArrayOfControlPointsAndGuidepoints(BufferedReader cFile, BezierSpline spline,
				...
				if (strLine.startsWith("gps")) {
					if (guidepointArray != null) {
						...
						while (strLine.startsWith("(gp")) {
							...
							guidepointArray.add(new Point2D.Double(Double.valueOf(values[0]).doubleValue(),
